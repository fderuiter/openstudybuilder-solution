...
<xsl:stylesheet version="1.0"
        ...
        <xsl:otherwise> <!-- Not a title -->
          ...
          <xsl:choose>
            <xsl:when test="MeasurementUnitRef">
              <div class="col-sm-3 {$labelColor} border text-left">
                <xsl:choose>
                  <xsl:when test="./@Origin = 'Derived Value'">
                    <input type="{@DataType}" class="{$labelColor} " name="{@Name}" min="4"  maxlength="40" size="{@Length}" aria-describedby="basic-addon2" disabled="disabled" />
                    &#160;<span class="text-center {$labelColor}" id="item{@OID}"><xsl:value-of select="@Length" /> digit(s)</span>
                  </xsl:when>
                  <xsl:otherwise>
                    <input type="{@DataType}" class="{$labelColor}" name="{@Name}" min="4"  maxlength="40" size="{@Length}" aria-describedby="basic-addon2" />
                    &#160;<span class="text-center {$labelColor}" id="item{@OID}"><xsl:value-of select="@Length" /> digit(s)</span>
                  </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
              ...
              <div class="col-sm-2 {$labelColor} border text-left">
              ...
              <div class="col-sm-2 {$labelColor} border text-center">
                <xsl:choose>
                  <xsl:when test="./Alias/@Context = 'CDASH/SDTM'">
                    <xsl:for-each select="./Alias[@Context = 'CDASH/SDTM']">
                      <xsl:call-template name="splitter">
                  ...
                  <xsl:for-each select="CodeListRef">
                    <div class="col-sm-6 {$labelColor} border text-left">
                        ...
                        <xsl:for-each select="//CodeList[@OID = current()/@CodeListOID]/EnumeratedItem">
                          <xsl:sort select="@OrderNumber" data-type="number" order="ascending" />
                          <input type="text" class="{$labelColor} " id="{@CodedValue}" name="{../@OID}" value="{@CodedValue}" />
                      ...
                      <xsl:when test="./@Origin = 'Protocol Value' or ./@Origin = 'Derived Value'">
                        <xsl:choose>
                          <xsl:when test="./Alias/@Context = 'DEFAULT_VALUE'">
                            <input type="{@DataType}" class="{$labelColor}" id="item{@OID}" name="{@Name}" min="4"  maxlength="40" size="{@Length}" aria-describedby="item{@OID}" placeholder="{./Alias[@Context = 'DEFAULT_VALUE']/@Name}" disabled="disabled" />
                            &#160;<span class="text-center {$labelColor}" id="item{@OID}"><xsl:value-of select="@Length" /> digit(s)</span>
                          </xsl:when>
                          <xsl:otherwise>
                            <input type="{@DataType}" class="{$labelColor}" id="item{@OID}" name="{@Name}" min="4"  maxlength="{@Length}" size="{@Length}" aria-describedby="item{@OID}" disabled="disabled" />
                            &#160;<span class="text-center {$labelColor}" id="item{@OID}"><xsl:value-of select="@Length" /> digit(s)</span>
                          </xsl:otherwise>
                        </xsl:choose>
                      </xsl:when>
                      <xsl:when test="./Alias/@Context = 'DEFAULT_VALUE'">
                        <input type="{@DataType}" class="{$labelColor}" id="item{@OID}" name="{@Name}" min="4"  maxlength="40" size="{@Length}" aria-describedby="item{@OID}" placeholder="{./Alias[@Context = 'DEFAULT_VALUE']/@Name}" aria-label="{@Name}" />
                        &#160;<span class="text-center {$labelColor}" id="item{@OID}"><xsl:value-of select="@Length" /> digit(s)</span>
                      </xsl:when>
                      <xsl:otherwise>
                        <input type="{@DataType}" class="{$labelColor}" id="item{@OID}" name="{@Name}" min="4"  maxlength="40" size="{@Length}" aria-describedby="item{@OID}" />
                        &#160;<span class="text-center {$labelColor}" id="item{@OID}"><xsl:value-of select="@Length" /> digit(s)</span>
                      </xsl:otherwise>
                    </xsl:choose>
                    <xsl:choose>
  ...
  <xsl:template match="FormDef" >
    ...
    <div class="content">
      <div class="odmForm">
        <div class="col">
          <h3><span class="badge-form">F</span>&#160;&#160;<xsl:value-of select="@Name" /></h3>
  ...
  <xsl:template name="IGsplitter">
    ...
    <div class="row sdtm collapse text-center" >
      <xsl:choose>
